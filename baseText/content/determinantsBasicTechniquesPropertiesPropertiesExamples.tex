...
$B = kA$. Then, $\det (B) = k^n \det (A)$. This gives the next theorem.
...
\begin{solution} 
By Definition~\ref{def:two-by-two-determinant}, $\det (A) =-2$. We can also compute
$\det (B)$ using Definition~\ref{def:two-by-two-determinant}, and we see that $\det (B) = -10$. 

Now, let's compute  $\det (B)$ using Theorem~\ref{thm:multiplying-row-by-scalar} and see if we
