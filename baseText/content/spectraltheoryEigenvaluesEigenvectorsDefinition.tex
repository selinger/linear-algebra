...
\textbf{eigenvalue}\index{eigenvalue}
of the matrix $A$ and $X$ is called an \textbf{eigenvector}\index{eigenvector} of $A$ associated with $\lambda$, or a $\lambda$-eigenvector of $A$. 
 
The set of all eigenvalues of an $n\times n$-matrix $A$ is denoted by 
$\sigma (A) $ and is referred to as the \textbf{spectrum}\index{spectrum} of $A$.
\end{definition}
...
Let's look at eigenvectors in more detail. Suppose $X$ satisfies {\eqref{eigen1}}. Then
\begin{equation*}
\begin{array}{c}
AX - \lambda X = 0 \\
\mbox{or} \\
...
\end{array}
\end{equation*}
for some $X \neq 0$. Equivalently you could write $(\lambda
I-A)X = 0$, which is more commonly used.  Hence, when we are looking for eigenvectors, we are
looking for non-trivial solutions to this homogeneous system of equations!
...
can conclude that
\begin{equation}
\det (\lambda I - A) =0  \label{eigen2}
\end{equation}
Note that this is equivalent to $\det (A- \lambda I) =0$. 

The expression $\det (\eigenVar I-A) $ is a polynomial (in
the variable $x$) called the
\textbf{characteristic polynomial of $A$}, and 
$\det (\eigenVar I-A) =0$ is called the \textbf{characteristic
...
polynomial are the eigenvalues of $A$.  Thus when {\eqref{eigen2}}
holds, $A$ has a non-zero eigenvector.

\begin{theorem}{The existence of an eigenvector}{existence-of-eigenvector}
Let $A$ be an $n\times n$-matrix and suppose $\det (\lambda I -
...
Expansion demonstrates that $\det (\lambda I - A) $ is a
polynomial of degree $n$. As such, the equation
{\eqref{eigen2}} has a solution $\lambda \in \C$ by the Fundamental
