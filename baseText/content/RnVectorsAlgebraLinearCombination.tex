\section{Linear combinations in \texorpdfstring{$\R^n$}{Rn}}
...
\begin{example}{Linear combination}{linearcombination}
  We have
  \begin{equation*}
    3
    \begin{mymatrix}{r}
...
\begin{solution}
  ...
  \end{equation*}
  In fact, it is also a linear combination of just $\vect{u}_1$ and
  $\vect{u}_2$.
\end{solution}
...
$\vect{v}$. On the other hand, if $\vect{u}$ and $\vect{v}$ are
non-parallel vectors in $\R^3$, their linear combinations form a
plane.
...
\draw (1.5,3.0) node[above left]{$0\vect{u}+3\vect{v}$};
\draw (3.0,3.5) node[above left]{$1\vect{u}+3\vect{v}$};
\draw (4.5,4.0) node[above left]{$1\vect{u}+3\vect{v}$};
...
\draw (2.5,0.0) node[below right]{$2\vect{u}-1\vect{v}$};
\draw (1.0,-0.5) node[below right]{$1\vect{u}-1\vect{v}$};
\draw (-0.5,-1.0) node[below right]{$0\vect{u}-1\vect{v}$};
\end{tikzpicture}
\end{center}
