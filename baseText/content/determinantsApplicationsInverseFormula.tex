...
\begin{definition}{The cofactor matrix}{cofactor-matrix}
  ...
  \index{cofactor matrix}, denoted $\func{cof}(A)$, is defined by
...
\index{adjugate} of $A$ to be the transpose of the cofactor matrix. We
can also call this matrix the \textbf{classical adjoint}%
\index{classical adjoint} of $A$, and we denote it by
$\adj (A)$.
...
%   in terms of expansion along column $r$ and multiplying by $\det(A)^{-1}$%
%   \index{inverses and determinants}, we get:
%   
%   \begin{equation*}
%     \sum_{i=1}^{n}a_{ir}
%     \cofactor{A}{ir}\det(A)^{-1}=\det(A)\det(A)^{-1}=1
%   \end{equation*}
%   Consider the following similar expression for $k\neq r$. 
%   \begin{equation*}
%     \sum_{i=1}^{n}a_{ir}\cofactor{A}{ik}\det(A)^{-1}
%   \end{equation*}
%   
...
%     \cofactor{A}{ik}\det(A) ^{-1}
%   \end{equation*}
%   Summarizing,
%   \begin{equation*}
%     \sum_{i=1}^{n}a_{ir}\cofactor{A}{ik}\det(A)
%     ^{-1}=\delta _{rk} = \left\{
%       \begin{array}{c}
          %           1\text{ if }r=k \\
          %           0\text{ if }r\neq k
