...
turns out that non-square matrices can never be invertible, as the
...
\begin{proof}
  Assume that $A$ and $B$ are matrices such that $AB=I$ and $BA=I$,
  and suppose that $A$ is an $m\times n$-matrix. Since both products
  $AB$ and $BA$ are defined, $B$ must be an $n\times m$-matrix. Assume
  $m\neq n$. Then we must either have $m<n$ or $n<m$.
  \begin{itemize}
  \item Case 1: $m<n$. In this case, $A$ has more columns than
    rows. It follows that the homogeneous system of equations
    $A\vect{x}=\vect{0}$ has a non-trivial solution; let $\vect{x}$ be
    such a solution. We obtain a contradiction by a similar method as
    in Example~\ref{exa:non-invertible-matrix}. Namely, we have
    $\vect{x}=I\,\vect{x} = (BA)\vect{x}=B(A\vect{x}) = B\vect{0} =
    \vect{0}$, contradicting the fact that $\vect{x}$ was
    non-trivial. Therefore, case 1 is impossible.
  \item Case 2: $n<m$. We can use exactly the same argument as in case
    1, but using the matrix $B$ instead of $A$. Therefore, case 2 is
    also impossible.
  \end{itemize}
  Since we got a contradiction from the assumption that $m\neq n$, it
  follows that $m=n$, i.e., $A$ and $B$ are square.
\end{proof}

We conclude this section by listing some algebraic properties of the inverse.
