...
\begin{proposition}{Solution of the subspace fitting problem}{subspace-fitting}
  ...
  \item By Proposition~\ref{prop:characterize-positive}, all
    ...
    $\vect{u}_1,\ldots,\vect{u}_n$ be the corresponding eigenvectors.
  \item Then $W=\sspan\set{\vect{u}_1,\ldots,\vect{u}_k}$ is the
    solution to the subspace fitting problem.
    Moreover, the total square distance of the points from the
    subspace is
    \begin{equation*}
      d(\vect{v}_1,W)^2 + \ldots + d(\vect{v}_m,W)^2
      = \eigenvar_{k+1} + \ldots + \eigenvar_n.
    \end{equation*}
  \end{enumerate}
\end{proposition}  
...
\begin{example}{Subspace fitting in $\R^2$}{subspace-fitting-r2}
  ...
  collection of points. What is the sum of the squares of the
  distances of the points from this subspace?
\end{example}
...
\begin{solution}
  ...
  \item The desired subspace $W$ is spanned by the eigenvector
    ...
    $W=\sspan\set{\vect{u}_1}$. The sum of the squares of the
    distances is $\lambda_2 = 27$.
  \end{enumerate}
...
\begin{definition}{Principal components}{principal-components}
  ...
  variability after $\vect{u}_1$ has been removed, and so on. 
\end{definition}
