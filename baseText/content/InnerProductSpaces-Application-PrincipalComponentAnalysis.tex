...
So far, we have been looking for a $k$-dimensional subspace of $\R^n$
that is as close as possible to a given collection of points. Every
subspace passes through the origin. But sometimes the points may not
be near the origin, as in this example:
\begin{equation}\label{eqn:subspace-fitting}
  ...
  \end{tikzpicture}
\end{equation}
In this case, looking for a subspace passing through the origin does
not make much sense. Instead, we should be looking for an
\textbf{affine subspace}. An affine subspace is similar to a subspace,
except it does not necessarily contain the origin.
...
$\R^n$. We can also describe a non-empty affine subspace as a subspace
that has been ``shifted'' away from the origin. The affine subspace
fitting problem is analogous to the subspace fitting problem:
...
\begin{definition}{Centroid}{centroid}
  ...
  \begin{equation*}
    \overline{\vect{v}} = \frac{1}{m}(\vect{v}_1+\ldots+\vect{v}_m).
  \end{equation*}
