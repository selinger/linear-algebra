...
\begin{proof}
  ...
  other. This is true because of trigonometric identities. For
  example,
  \begin{eqnarray*}
    \iprod{g_i,g_j}
    &=& \int_{0}^{2\pi} \cos(ix)\cos(jx)\,dx \\
    &=& \frac{1}{2}\int_{0}^{2\pi} \cos((i-j)x) + \cos((i+j)x)\,dx \\
    &=& \bigbracket{\frac{1}{2(i-j)} \sin((i-j)x) + \frac{1}{2(i+j)}
      \sin((i+j)x)}_{0}^{2\pi} \\
    &=& 0
  \end{eqnarray*}
  when $i,j\geq 0$ and $i\neq j$. The cases for $\iprod{f_i,f_j}$ and
  $\iprod{f_i,g_j}$ are similar.
\end{proof}
