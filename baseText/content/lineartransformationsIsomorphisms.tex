...
\item[B.]  Determine if two subspaces of $\R^n$ are isomorphic. 
\end{enumerate}
\end{outcome}
...
\end{mymatrix} \\
&=& 
k T \left(  \begin{mymatrix}{c}
x_1 \\
y_1
\end{mymatrix} \right) + p T \left( \begin{mymatrix}{c}
x_2 \\
y_2
\end{mymatrix} \right)
\end{eqnarray*}
...
x \\
y
\end{mymatrix} \right) = \begin{mymatrix}{c}
x + y\\
x - y 
\end{mymatrix} = \begin{mymatrix}{c}
0 \\
...
because if $\vect{y}\in \R^{n},A\left( A^{-1} (\vect{y})\right) =\left(
...
subspaces of $\R^n$. If $T$ is one to one, then it has the property that if $\left\{ \vect{u}_{1},\cdots ,\vect{u}_{k}\right\} $ is linearly independent, so is $\left\{ T(\vect{u}_{1}),\cdots ,T(\vect{u}_{k})\right\} $.
...
Let $V=\R^{3}$ and let $W$ denote 
\begin{equation*}
\func{span}\left\{ \begin{mymatrix}{c}
1 \\ 
2 \\ 
1 \\ 
1
