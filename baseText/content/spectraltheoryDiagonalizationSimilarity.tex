...
be
diagonalizable\index{diagonalizable}. 
We define a diagonal matrix $D$ as a matrix containing a zero in every entry 
except those on the main diagonal\index{matrix!diagonal matrix}\index{matrix!main diagonal}. More precisely, if $d_{ij}$ is the $\ijth$ entry of a diagonal matrix $D$, then
...
Let $A$ be an $n\times n$-matrix. Then $A$ is said to be \textbf{diagonalizable}\index{diagonalizable} if there exists an invertible matrix $P$ such that
\begin{equation*}
P^{-1}AP=D
\end{equation*}
