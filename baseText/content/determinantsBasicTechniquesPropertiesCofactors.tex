...
Let $A$ be a $3\times 3$ matrix. The $ij^{th}$ \textbf{minor}\index{determinant!minor} of $A$, denoted as $minor\left( A\right) _{ij},$ is the determinant
...
Suppose $A$ is an $n\times n$ matrix. The $ij^{th}$ \textbf{cofactor}\index{determinant!cofactor}, denoted by $\func{cof}\left( A\right) _{ij}$ is
defined to be 
\begin{equation*}
