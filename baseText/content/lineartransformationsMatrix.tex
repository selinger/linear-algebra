...
Let $T: \R^{n} \to \R^{m}$ be a linear transformation. Then the matrix $A$ satisfying $T\tup{\vect{x}}=A\vect{x}$\index{linear transformation!matrix} is given by
\begin{equation*}
A=
\begin{mymatrix}{ccc}
| &  & | \\
...
$\proj_{\vect{u}}\tup{\vect{v}} $ which takes a vector a transforms it to its projection onto a given vector $\vect{u}$. It turns out that
this map is linear, a result which follows from the properties of the
dot product. This is shown as follows.
\begin{eqnarray*}
\proj_{\vect{u}}\tup{k \vect{v}+ p \vect{w}}
...
&=& k \; \proj_{\vect{u}}\tup{\vect{v}} +p \; \proj
_{\vect{u}}\tup{\vect{w}} 
\end{eqnarray*}
...
\begin{enumerate}
\item
First, we have just seen that $T (\vect{v}) = \proj_{\vect{u}}\tup{\vect{v}}$ is linear. Therefore by Theorem~\ref{thm:matrix-lin-transf}, we can find a matrix $A$ such that $T(\vect{x}) = A\vect{x}$. 
...
\begin{equation*}
\proj_{\vect{u}}\tup{\vect{e}_{i}} = \tup{\vspace{0.05in}
