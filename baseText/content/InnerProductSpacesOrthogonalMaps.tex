...
\begin{proposition}{The matrix of an orthogonal transformation}{matrix-orthogonal}
  Let $V,W$ be inner product spaces, let $T:V\to W$ be a linear
  transformation. Let $B=\set{\vect{v}_1,\ldots,\vect{v}_m}$ be an
  orthonormal basis of $V$ and let
  $C=\set{\vect{w}_1,\ldots,\vect{w}_n}$ be an orthonormal basis of
  $W$. Let $P=\coord{T}_{C,B}$ be the matrix of $T$ with respect to
  the bases $B$ and $C$. Then
  \begin{enumialphparenastyle}
    \begin{enumerate}
    \item $T$ is an isometry if and only if $P^TP = I$.
    \item $T$ is orthogonal if and only if $P^TP = I$ and $n=m$.
    \end{enumerate}
...
\begin{solution}
  ...
  nor orthogonal. A similary pair of calculations shows that $C^TC=I$
