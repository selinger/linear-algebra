...
q_{1},q_{2},q_{3}\right) $ be two points in $\R^{3}.$ Consider the
...
\end{equation*}
This is called the \textbf{distance formula}\index{distance formula}. We may also write $\left\vert P - Q \right\vert $ as the distance between $P$ and $Q$.
\end{definition}
...
Describe the points in $\R^3$ which are equally distant from both the points $\left(
...
$\R^n$. Then, the length of $\vect{u}$, written $\vectlength \vect{u} \vectlength$\index{vector!length}, is given by
\begin{equation*}
\vectlength
\vect{u}
\vectlength
...
\textbf{unit vector}\index{unit vector}\index{vector!unit} if it has length 1, that is if
\begin{equation*}
