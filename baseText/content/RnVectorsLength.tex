...
\begin{outcome}
  \begin{enumerate}
    \item[A.] Compute the distance between points in $n$-dimensional space.
    \item[B.] Compute the length of a vector algebraically and
      geometrically.
    \item[C.] Find vectors that are a given distance from other vectors.
    \item[D.] Use algebraic properties of the length operation to
      prove equalities.
    \item[E.] Normalize a vector.
\end{enumerate}
\end{outcome}
...
\begin{definition}{Distance between points}{distance-between-points}
  ...
  two points in $\R^{n}$. Then the \textbf{distance}\index{distance}
...
\begin{definition}{Length of a vector}{length-of-vector}
  ...
  \textbf{magnitude}\index{magnitude}\index{vector!magnitude}.
\end{definition}
...
\begin{definition}{Unit vector}{unit-vector}
  ...
  \textbf{unit vector}\index{unit vector}\index{vector!unit} if it has
  length $1$, that is, if
  \begin{equation*}
    \norm{\vect{u}} = 1.
  \end{equation*}
...
\vect{u} = \frac{1}{\norm{\vect{v}}} \vect{v}.
\end{equation*}
We often use the term \textbf{normalize}\index{vector!normalize} to refer
