...
Recall that a function is simply a transformation of a vector to
result in a new vector. Consider the following definition.

\begin{definition}{Linear transformation}{linear-transformation}
  Let $V$ and $W$ be vector spaces. Suppose $T: V \to W$ is a
  function, where for each $\vect{x} \in V ,T(\vect{x})\in W$. Then
  $T$ is a \textbf{linear transformation}\index{linear transformation}
  if whenever $k ,p $ are scalars and $\vect{v}_1$ and $\vect{v}_2$
  are vectors in $V$
  \begin{equation*}
    T(k \vect{v}_1 + p \vect{v}_2) = kT(\vect{v}_1)+ pT(\vect{v}_{2})
  \end{equation*}
\end{definition}
