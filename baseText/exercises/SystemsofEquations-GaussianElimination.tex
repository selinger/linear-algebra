...
\begin{ex}
  ...
  \begin{sol}
    It is because you cannot have more than $\min\tup{m,n}$ pivot
    entries in the {\ef}. Recall that each row of the matrix can have
    at most one pivot entry (the first non-zero entry of that
    row). Therefore, there are no more than $m$ pivot entries. Also,
    by the definition of {\ef}s, each pivot entry is in a separate
    column. So there are no more than $n$ pivot entries. 
  \end{sol}
\end{ex}
