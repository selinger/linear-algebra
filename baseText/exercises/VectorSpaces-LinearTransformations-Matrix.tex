...
\[
T\vect{x}=T\tup{\sum_{i=1}^{n}y_{i}\vect{a}_{i}}
=\sum_{i=1}^{n}y_{i}T\vect{a}_{i}=\sum_{i=1}^{n}y_{i}A\vect{a}
_{i}=A\tup{\sum_{i=1}^{n}y_{i}\vect{a}_{i}} =A\vect{x}
\]
...
\begin{ex}  Find the matrix for $T\tup{\vect{w} } = \proj_{\vect{v}}\tup{\vect{w}} $
where $\vect{v}=\begin{mymatrix}{rrr}
1 & -2 & 3
\end{mymatrix} ^{T}$.
\begin{sol}
 Recall that $\proj_{\vect{u}}\tup{\vect{v}} =\frac{\vect{v}\dotprod\vect{u} }{\norm{\vect{u}} ^{2}}\vect{u}$ and so the desired matrix
has $i^{th}$ column equal to $\proj_{\vect{u}}\tup{\vect{e}_{i}}$. Therefore, the matrix desired is
\[
